...
const playerToggler = () => {
    ...
    if ($(playerContainer).hasClass("clicked")) $(".controls").children().each(() => $(this).attr("tabindex", "0"))
    else $(".controls").children().each(() => $(this).attr("tabindex", "-1"))
...
const mudarPersonagem01 = () => {
    ...
    $(jogador).prop("src", p1);
    $(msgPersonagemSelecionado).css("opacity", "1");
    $(btnSelectP1).addClass("selecionado");
    setTimeout(() => $(btnSelectP1).removeClass("selecionado"), 500);
    setTimeout(() => $(msgPersonagemSelecionado).css("opacity", "0"), 1000);
};
...
const mudarPersonagem02 = () => {
    ...
    setTimeout(() => $(btnSelectP2).removeClass("selecionado"), 500);
    setTimeout(() => $(msgPersonagemSelecionado).css("opacity", "0"), 1000);
};
...
const mensagemDificuldade = () => {
    $(msgDificuldadeSelecionada).css("opacity", "1");
    setTimeout(() => $(msgDificuldadeSelecionada).css("opacity", "0"), 1000);
};
...
const modoFacil = () => {
    ...
    if ($(btnModoMedio).hasClass("selected")) $(btnModoMedio).removeClass("selected");
    if ($(btnModoDificil).hasClass("selected")) $(btnModoDificil).removeClass("selected");
};
...
const modoMedio = () => {
    ...
    if ($(btnModoFacil).hasClass("selected")) $(btnModoFacil).removeClass("selected");
    if ($(btnModoDificil).hasClass("selected")) $(btnModoDificil).removeClass("selected");
};
...
const verificaJogo = () => {
    ...
    this.aleatoryWidthSrc = setInterval(() => {
    ...
    if ($(btnModoFacil).hasClass("selected")) {
        ...
        this.interval1 = setInterval(() => {
            $(darkScreen).css("animation", "6s ease blink2");
            setTimeout(() => $(darkScreen).css("animation", "none"), 10000);
        }, 20000);
    }
    else if ($(btnModoDificil).hasClass("selected")) {
        $(darkScreen).css("z-index", "4").css("display", "flex");
        this.interval2 = setInterval(() => {
            $(darkScreen).css("animation", "4s ease blink2");
            setTimeout(() => $(darkScreen).css("animation", "none"), 12000);
        }, 15000);
    }
    else {
        $(darkScreen).css("display", "none").css("animation", "none");
    ...
    this.loop = setInterval(() => {
        ...
        if (!$(gameBoard).hasClass("resized") && margemEsquerdaObstaculo <= 90 && margemEsquerdaObstaculo > 0 && alturaPulo <= 50
            || $(gameBoard).hasClass("resized") && margemEsquerdaObstaculo <= 120 && margemEsquerdaObstaculo > 0 && alturaPulo <= 50) {
            ...
            clearInterval(aleatoryWidthSrc);
            ...
            $(listaDeTempos).append($(el));
            $(listaDeTempos).children().each(function () {
                arr.push($(this).html()); // a array pegará o texto dos elementos da lista de histórico de tempos
            });
